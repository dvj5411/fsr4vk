...
for tier in ('1080','2160'):
 for preset in a.presets:
  ...
  for i,entry in enumerate(source['shaders']):
   ...
   payloads.append(raw)
  validated.append((tier,preset,dict(schema=1,capacity=source['capacity'],preset=preset,shaders=entries,provenance=json.loads((a.captures/'provenance.json').read_text())),payloads))
...
print(f'Imported {len(validated)*15} portable shaders; ABI and Vulkan 1.1/1.3 validation passed.')
