#!/usr/bin/env python3
"""Package normal/debug providers and the matching OptiScaler build."""
import argparse
...
parser.add_argument('--provider-debug', type=Path, required=True)
...
files = {
    ...
    'OptiScaler/amd_fidelityfx_upscaler_vk_debug.dll': args.provider_debug,
    ...
    'LICENSES/Zstandard-BSD.txt': root / 'LICENSES/Zstandard-BSD.txt',
}
for name, path in files.items():
    if not path.is_file():
        parser.error(f'missing input for {name}: {path}')
for path in (args.provider, args.optiscaler, args.provider_debug):
