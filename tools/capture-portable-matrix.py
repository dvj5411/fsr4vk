...
for tier,ow,oh,inputs in [('1080',1920,1080,[(1920,1080),(1280,720),(1129,635),(960,540),(640,360),(1280,720)]),('2160',3840,2160,[(3840,2160),(2560,1440),(2258,1270),(1920,1080),(1280,720),(2560,1440)])]:
 ...
 for preset,(rw,rh) in zip(('native','quality','balanced','performance','ultraperf','drs'),inputs):
  ...
  case={'case':tier+'-'+preset,'input':[rw,rh],'output':[ow,oh],'tier':tier,'preset':preset,'runs':[str(ref)],'shader_dump':str(dump),'dynamic_resolution':preset=='drs'}
  if not (ref/'frame-001.rgba16f').exists():
   e=os.environ.copy();e.update(STEAM_COMPAT_DATA_PATH=str(a.compat_data.resolve()),STEAM_COMPAT_CLIENT_INSTALL_PATH=str(a.steam_root.resolve()),SteamAppId='0',SteamGameId='0',VK_LOADER_DRIVERS_SELECT=a.driver_filter,WINEDEBUG='-all',VKD3D_CONFIG='force_raw_va_cbv',VKD3D_DISABLE_EXTENSIONS=a.disable_extensions,VKD3D_SHADER_DUMP_PATH=str(dump),VKD3D_SHADER_CACHE_PATH='0')
