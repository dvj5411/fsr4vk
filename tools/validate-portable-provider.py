...
p.add_argument('--expected-backend',default='portable-int8',choices=['portable-int8','native-mixed-dot'])
a=p.parse_args()
...
for c in json.loads(a.cases.read_text())['cases']:
 ...
 e.pop('FSR4_TEST_DYNAMIC_RESOLUTION',None);e.pop('FSR4_TEST_SECOND_RENDER_SIZE',None)
 if c.get('dynamic_resolution'):e['FSR4_TEST_DYNAMIC_RESOLUTION']='1'
 if c.get('second_input'):e['FSR4_TEST_SECOND_RENDER_SIZE']='x'.join(map(str,c['second_input']))
 ...
 try:
  launcher=[str(proton),'run','cmd.exe','/d','/c',command]
  if a.runner=='umu':
   ...
   launcher=['umu-run','cmd.exe','/d','/c',command]
  with (out/(name+'.proton.log')).open('w') as f:
   result=subprocess.run(launcher,env=e,cwd=exe.parent,stdout=f,stderr=subprocess.STDOUT,timeout=100)
  assert result.returncode==0, f'exit {result.returncode}'
  assert 'shader_backend='+a.expected_backend in diag.read_text(), 'missing backend dispatch proof'
  if c.get('dynamic_resolution'):assert 'preset=drs' in diag.read_text(), 'DRS model not selected'
