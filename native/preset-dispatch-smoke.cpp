...
#include "../provider/ffx_vk_preset_query.h"
...
int main(int argc, char** argv) {
    // Preserve progress if a driver/Proton test times out during teardown.
    ...
    auto ffxConfigure = reinterpret_cast<PfnFfxConfigure>(GetProcAddress(module,"ffxConfigure"));
    ...
    if (!ffxCreateContext || !ffxDispatch || !ffxDestroyContext || !ffxConfigure || !ffxQuery) return 1;
#endif
    ffxContext api_context = nullptr;
    unsigned preset_test_dispatches = 0;
    ...
    try {
        ...
        const auto record_context = [&] {
            ...
            const char* requested = std::getenv("FSR4_TEST_PRESET");
            if (preset_test_dispatches > 0 && std::getenv("FSR4_TEST_NEXT_PRESET"))
                requested = std::getenv("FSR4_TEST_NEXT_PRESET");
            uint32_t expected_preset = FSR4VK_PRESET_UNKNOWN;
            if (requested) {
                Fsr4VkConfigurePreset configure{{FSR4VK_CONFIGURE_DESC_TYPE_PRESET, nullptr}, FSR4VK_PRESET_AUTO};
                if (std::strcmp(requested, "auto") != 0) {
                    if (std::strlen(requested) != 1 || requested[0] < '0' || requested[0] > '5')
                        throw std::runtime_error("invalid test preset");
                    configure.preset = static_cast<uint32_t>(requested[0] - '0');
                }
                if (ffxConfigure(&api_context, &configure.header) != FFX_API_RETURN_OK)
                    throw std::runtime_error("preset configure rejected");
                expected_preset = configure.preset;
                if (expected_preset == FSR4VK_PRESET_AUTO) {
                    const std::string automatic = fsr4::resolution_preset(kRenderWidth, kOutputWidth);
                    expected_preset = automatic == "native" ? 0 : automatic == "quality" ? 1 :
                                      automatic == "balanced" ? 2 : automatic == "performance" ? 3 : 5;
                }
            }
            if (ffxDispatch(&api_context,&d.header)!=FFX_API_RETURN_OK)
            ...
            if (requested) {
                ...
                if (ffxQuery(&api_context, &query.header) != FFX_API_RETURN_OK || query.activePreset != expected_preset)
                    throw std::runtime_error("forced preset report mismatch");
                std::cout << "preset_frame=" << preset_test_dispatches << " requested=" << requested
                          << " active=" << query.activePreset << '\n';
            }
            ++preset_test_dispatches;
        };
        if (use_context) {
            if (use_provider) {
    ...
    context.reset();
    if (api_context) ffxDestroyContext(&api_context,nullptr);
    std::cout << "cleanup=provider-destroyed\n";
    for (auto pipeline : pipelines) vkDestroyPipeline(device, pipeline, nullptr);
    for (auto module : modules) vkDestroyShaderModule(device, module, nullptr);
